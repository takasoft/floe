...
def parse_dit_sql(sql_text: str, source_path: str | None = None) -> DynamicTable:
    ...
    if not m:
        raise ValueError(
            f"Could not parse DIT statement. Expected 'CREATE DYNAMIC TABLE <name> [LAG = '...'] [REFRESH_MODE = '...'] AS <query>'"
        )

    name = m.group("name").strip()
    options_str = m.group("options") or ""
    query = m.group("query").strip()

    options = {k.lower(): v for k, v in _OPTION_RE.findall(options_str)}
